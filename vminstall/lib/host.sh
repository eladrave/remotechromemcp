...
vm_check_host() {
  local required
  vm_require_root
  for required in getent curl ss tar sha256sum; do
...
vm_verify_dns() {
  ...
  local resolved_output external_ip address normalized_external
  ...
  while read -r address _; do
    [[ $(vm_normalize_address "$address") == "$normalized_external" ]] &&
      return 0
